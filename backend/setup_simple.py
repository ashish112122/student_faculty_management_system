"""
Simple Database Setup - Creates tables directly in Python
"""
import oracledb
...
# Insert marks for student 1 (Rohan Sharma)
marks_data = [
    (1, 1, 1, 'MST', 38, 50),
    (2, 1, 1, 'EST', 72, 100),
    (3, 1, 1, 'Assignment', 18, 20),
    (4, 1, 1, 'Quiz', 9, 10),
    (5, 1, 2, 'MST', 42, 50),
    (6, 1, 2, 'EST', 78, 100),
    (7, 1, 2, 'Assignment', 19, 20),
    (8, 1, 2, 'Quiz', 8, 10),
]
...
for subject_id in range(1, 6):
    for day in range(30):
        status = 'P' if day % 4 != 0 else 'A'  # 75% attendance
...
cursor.executemany("""
    INSERT INTO attendance (attendance_id, student_id, subject_id, attendance_date, status)
    VALUES (:1, :2, :3, :4, :5)
""", attendance_data)
...
alerts_data = [
    (1, 1, 'Warning', 'Your attendance in DBMS is below 75%'),
    (2, 1, 'Info', 'Assignment deadline approaching for OS'),
    (3, 1, 'Critical', 'Your attendance in CN is below 65%'),
]
