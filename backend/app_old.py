...
def student_dashboard():
    ...
    try:
        ...
        # Calculate total credits (assume 4 credits per subject)
...
def get_student_marks():
    ...
    try:
        ...
        cursor.execute("""
            SELECT s.subject_name, s.subject_code, m.assessment_type, m.marks_obtained, m.max_marks,
                   (SELECT AVG(marks_obtained) FROM marks WHERE subject_id = m.subject_id 
                    AND assessment_type = m.assessment_type) as class_avg
            FROM marks m
            JOIN subjects s ON m.subject_id = s.subject_id
            WHERE m.student_id = :student_id
            ORDER BY s.subject_name, m.assessment_type
        """, {'student_id': student_id})
        
        marks = []
        for row in cursor.fetchall():
            percentage = round((row[3] / row[4]) * 100, 2)
            ...
            marks.append({
                ...
                'marks_obtained': float(row[3]),
                'max_marks': float(row[4]),
                'percentage': percentage,
                'grade': grade,
                'class_average': round(float(row[5]), 2)
            })
...
def get_student_attendance():
    ...
    try:
        ...
        attendance = []
        for row in cursor.fetchall():
            percentage = round((row[3] / row[2]) * 100, 2) if row[2] > 0 else 0
            status = 'Critical' if percentage < 50 else 'Warning' if percentage < 75 else 'Good'
